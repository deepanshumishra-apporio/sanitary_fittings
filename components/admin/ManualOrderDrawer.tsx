...
export default function ManualOrderDrawer({ onClose, onCreated }: Props) {
  ...
  useEffect(() => {
    if (!selectedCategoryId) { setCategoryProducts([]); return; }
  ...
  async function lookupByEmail() {
    ...
    try {
      const { data } = await api.get<{ data: Array<{ id: string; name: string | null; phone: string | null }> }>(
        `/auth/users?search=${encodeURIComponent(email)}&limit=1`
      );
      const match = data.data.find(
        (u) => "email" in u && (u as { email?: string }).email === email
      );
      if (match) {
  ...
  const searchProducts = useCallback(async (q: string) => {
    ...
    try {
      const { data } = await api.get<{ data: Product[] }>(
        `/product?search=${encodeURIComponent(q)}&limit=8`
      );
      ...
      setCart((prev) =>
        prev.map((e) =>
          e.product.id === productId
            ? { ...e, quantity: Math.min(qty, e.product.stock) }
            : e
        )
      );
    }
  ...
  const productDiscountAmount = cart.reduce(
    (s, e) => s + (e.product.price * Math.max(0, e.product.discount) / 100) * e.quantity,
    0
  );
  const discountedSubtotal = Math.max(0, subtotal - productDiscountAmount);
  const manualDiscountAmount = discountType === "percent"
    ? discountedSubtotal * ((parseFloat(discountValue) || 0) / 100)
    : parseFloat(discountValue) || 0;
  const discountNum = parseFloat(discountValue) || 0;
  const discountAmount = Math.min(discountedSubtotal, productDiscountAmount + manualDiscountAmount);
  const total = Math.max(0, discountedSubtotal - manualDiscountAmount);
      ...
      <div className="flex max-h-[92vh] w-full flex-col overflow-hidden rounded-t-2xl border border-zinc-200 bg-white shadow-2xl sm:h-full sm:max-h-none sm:max-w-lg sm:rounded-none sm:border-y-0 sm:border-r-0 sm:border-l">
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
          <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <Field label="Full Name (optional)" value={customerName} onChange={setCustomerName} placeholder="Jane Doe" />
                <Field label="Phone (optional)" value={customerPhone} onChange={setCustomerPhone} placeholder="+91 98765 43210" type="tel" />
              </div>
            </div>
            ...
            <div>
              <div className="mb-3 flex items-center justify-between">
                <h3 className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-zinc-400">
              ...
              {!includeAddress ? (
                <p className="py-2 text-center text-xs text-zinc-400">Address is optional — tick "Include" to add one</p>
              ) : (
                <div className="space-y-3">
                  <Field
            ...
            <div>
              ...
              {/* Step 2 — Products from selected category */}
              {selectedCategoryId && (
                <div className="mb-3 max-h-48 overflow-y-auto border border-zinc-100 bg-white shadow-sm">
                  {loadingCategory ? (
                    ...
                    categoryProducts.map((p) => (
                      ...
                      >
                        {p.images[0] ? (
                          <img src={p.images[0]} alt={p.name} className="h-9 w-9 shrink-0 object-cover" />
                        ) : (
                          <div className="h-9 w-9 shrink-0 bg-zinc-100" />
                        )}
                        <div className="min-w-0 flex-1 text-left">
                          <p className="truncate text-sm font-semibold text-zinc-900">{p.name}</p>
                          <p className="text-xs text-zinc-400">{fmtPrice(p.price)} · {p.stock} in stock</p>
                        </div>
              ...
              <div className="relative mb-3">
                ...
                <input
                  ...
                  placeholder="Or search by name across all categories…"
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1 text-left">
                          ...
                          <p className="text-xs text-zinc-400">
                            {fmtPrice(p.price)} · {p.stock} in stock · {p.category.name}
                          </p>
                        </div>
                        <span className="shrink-0 text-xs font-bold text-orange-500">+ Add</span>
                      </button>
                  ...
                  {cart.map((entry) => (
                    <div key={entry.product.id} className="flex items-center gap-3 border border-zinc-100 bg-zinc-50 px-3 py-2.5">
                      {entry.product.images[0] ? (
                        <img src={entry.product.images[0]} alt={entry.product.name} className="h-9 w-9 shrink-0 object-cover" />
                      ) : (
                        <div className="h-9 w-9 shrink-0 bg-zinc-200" />
                      )}
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-semibold text-zinc-900">{entry.product.name}</p>
                        <p className="text-xs text-zinc-400">
                          {entry.product.discount > 0
                            ? `${fmtPrice(entry.product.price * (1 - entry.product.discount / 100))} each`
                            : `${fmtPrice(entry.product.price)} each`}
                        </p>
                      </div>
                      <div className="flex items-center gap-1">
                        <button
                          type="button"
                          onClick={() => setQty(entry.product.id, entry.quantity - 1)}
                          className="flex h-6 w-6 items-center justify-center border border-zinc-200 text-sm text-zinc-600 hover:border-zinc-900"
                        >
                          −
                        </button>
                        ...
                        <button
                          ...
                          className="flex h-6 w-6 items-center justify-center border border-zinc-200 text-sm text-zinc-600 hover:border-zinc-900 disabled:opacity-30"
                        >
                          +
                        </button>
                      </div>
                      <span className="w-20 text-right text-sm font-bold text-zinc-900">
                        {fmtPrice((entry.product.price * (1 - Math.max(0, entry.product.discount) / 100)) * entry.quantity)}
                      </span>
                      <button
                        type="button"
                        onClick={() => setCart((prev) => prev.filter((e) => e.product.id !== entry.product.id))}
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="shrink-0 border border-zinc-200 bg-white px-2 py-2 text-sm text-zinc-700 focus:border-zinc-900 focus:outline-none"
                >
              ...
              {discountAmount > 0 && (
                <p className="mt-1.5 text-xs font-medium text-emerald-600">
                  Discount applied: −{fmtPrice(discountAmount)}
                  {manualDiscountAmount > 0 && discountType === "percent" && ` (${discountNum}%)`}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="shrink-0 border-t border-zinc-100 bg-white px-6 py-4">
            {error && (
              <p className="mb-3 border border-red-100 bg-red-50 px-3 py-2 text-xs text-red-600">
                {error}
              </p>
            )}
            ...
            <div className="flex gap-3">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 border border-zinc-200 py-2.5 text-xs font-semibold uppercase tracking-widest text-zinc-600 transition-colors hover:border-zinc-900 hover:text-zinc-900"
              >
                Cancel
              </button>
              <button
